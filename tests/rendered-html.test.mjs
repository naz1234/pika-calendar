...
test("ships automatic shared sync, roster import, and installable app assets", async () => {
  ...
  assert.match(styles, /\.day-cell\.today\s*\{[^}]*animation:\s*today-tile-pulse 1\.65s ease-in-out infinite;[^}]*will-change:\s*box-shadow;/s);
  assert.match(styles, /@keyframes today-tile-pulse\s*\{[\s\S]*?0 0 16px[\s\S]*?50%\s*\{[\s\S]*?0 0 24px/s);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.day-cell\.today\s*\{[^}]*animation:\s*none !important;[^}]*will-change:\s*auto;/s);
