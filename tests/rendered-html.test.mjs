...
test("ships automatic shared sync, roster import, and installable app assets", async () => {
  ...
  assert.doesNotMatch(source, /sharedRosterOpen|setSharedRosterOpen|shared-roster-panel/);
  assert.match(source, /className="menu-row shared-roster-action"/);
  assert.match(source, /onClick=\{openRosterArchive\}/);
  assert.match(source, /aria-haspopup="dialog"/);
  assert.match(source, /aria-controls="roster-archive-dialog"/);
  assert.match(source, /id="roster-archive-dialog"/);
  assert.match(styles, /\.shared-roster-action/);
  assert.doesNotMatch(styles, /shared-roster-toggle|shared-roster-panel|roster-archive-trigger/);
  assert.match(styles, /--menu-file-sub-bg:/);
  assert.match(styles, /\.shared-roster-row\s*\{[^}]*background:\s*var\(--menu-file-sub-bg\);/s);
  ...
  assert.match(source, /\(!menuOpen && !rosterArchiveOpen\)/);
  ...
  const manualEditorMarkup = source.slice(personalEditorStart, source.indexOf('<label className="field"><span>Notes', personalEditorStart));
  ...
  assert.match(styles, /\.day-cell\.shift-event:is\(\.today, \.selected\):is\(\.shift-run-continues-previous, \.shift-run-continues-next\)\s*\{[^}]*animation:\s*none;[^}]*border-color:\s*transparent;[^}]*box-shadow:\s*none;/s);
  assert.match(styles, /\.day-cell\.shift-event:is\(\.today, \.selected\):is\(\.shift-run-continues-previous, \.shift-run-continues-next\) \.day-hit::before\s*\{[^}]*inset:\s*1px;[^}]*border-radius:\s*9px;[^}]*animation:\s*joined-date-tile-pulse 1\.65s ease-in-out infinite;/s);
  assert.match(styles, /@keyframes joined-date-tile-pulse\s*\{[\s\S]*?inset 0 0 18px[\s\S]*?50%\s*\{[\s\S]*?inset 0 0 30px/s);
  assert.doesNotMatch(styles, /date-marker-pulse/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.day-cell\.shift-event:is\(\.today, \.selected\):is\(\.shift-run-continues-previous, \.shift-run-continues-next\)\s*\{[^}]*box-shadow:\s*none;[^}]*\}[\s\S]*?\.day-cell\.shift-event:is\(\.today, \.selected\):is\(\.shift-run-continues-previous, \.shift-run-continues-next\) \.day-hit::before\s*\{[^}]*animation:\s*none !important;[^}]*inset 0 0 20px/s);
  ...
  assert.match(serviceWorker, /my-calendar-v49/);
...
test("uses a spacious icon-led calendar sidebar", async () => {
  const [source, styles] = await Promise.all([
    readFile(path.join(projectRoot, "app", "page.tsx"), "utf8"),
    readFile(path.join(projectRoot, "app", "globals.css"), "utf8"),
  ]);

  assert.match(source, /className="menu-action-icon menu-action-icon-import"/);
  assert.match(source, /className="menu-action-icon menu-action-icon-shared"/);
  assert.match(source, /className="menu-action-icon menu-action-icon-external"/);
  assert.match(source, /className="menu-action-icon menu-action-icon-settings"/);
  assert.match(source, /className="menu-action-copy"><strong>Import roster file<\/strong>/);
  assert.match(source, /onClick=\{chooseRosterFile\}/);
  assert.match(source, /className="menu-row shared-roster-action"[\s\S]*?onClick=\{openRosterArchive\}/);
  assert.match(source, /setSettingsOpen\(\(current\) => !current\)/);
  assert.match(styles, /\.menu-drawer\s*\{[^}]*width:\s*min\(68vw, 300px\);[^}]*background:\s*color-mix\(in srgb, var\(--surface-raised\) 88%, var\(--bg\)\);/s);
  assert.match(styles, /\.menu-drawer \.menu-import-row,[\s\S]*?\.menu-drawer \.shared-roster-action,[\s\S]*?\.menu-drawer \.settings-toggle\s*\{[^}]*grid-template-columns:\s*30px minmax\(0, 1fr\) 24px;[^}]*min-height:\s*76px;[^}]*border:\s*0;[^}]*background:\s*transparent;/s);
  ...
  assert.match(styles, /\.menu-drawer \.settings-section\s*\{[^}]*border-bottom:\s*0;/s);
});
