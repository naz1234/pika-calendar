...
test("ships automatic shared sync, roster import, and installable app assets", async () => {
  ...
  assert.match(serviceWorker, /my-calendar-v41/);
...
test("uses a spacious icon-led calendar sidebar", async () => {
  ...
  assert.match(styles, /\.menu-drawer\s*\{[^}]*width:\s*min\(74vw, 330px\);[^}]*background:\s*color-mix\(in srgb, var\(--surface-raised\) 88%, var\(--bg\)\);/s);
