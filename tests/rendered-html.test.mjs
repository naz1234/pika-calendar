...
test("ships automatic shared sync, roster import, and installable app assets", async () => {
  ...
  assert.match(styles, /\.salary-setting-control input\s*\{[^}]*text-align:\s*right;/s);
  ...
  assert.match(styles, /\.day-cell\.today::after\s*\{[^}]*border:\s*1px solid var\(--accent\);[^}]*animation:\s*today-tile-pulse 2\.4s ease-out infinite;/s);
  assert.match(styles, /@keyframes today-tile-pulse\s*\{[\s\S]*?transform:\s*scale\(0\.88\);[\s\S]*?transform:\s*scale\(1\);/s);
