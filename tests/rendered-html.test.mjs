...
test("uses the centered swipe-first calendar header", async () => {
  ...
  assert.match(styles, /\.month-swipe-viewport\s*\{[^}]*overflow:\s*hidden;[^}]*touch-action:\s*pan-y;/s);
  assert.match(styles, /\.month-swipe-track\s*\{[^}]*display:\s*flex;[^}]*width:\s*300%;[^}]*transform:\s*translate3d\(-33\.333333%, 0, 0\);[^}]*will-change:\s*transform;/s);
  assert.match(styles, /\.month-swipe-track > \.month-card\s*\{[^}]*flex:\s*0 0 33\.333333%;[^}]*width:\s*33\.333333%;/s);
